...
%>
  <tr>
    ...
    <td><%=xianlubianhao %></td><td><%=xianlumingcheng %></td><td><%=chufadi %></td><td><%=mudedi %></td><td><%=jiaotonggongju %></td><td><%=feiyong %></td><td><%=chuxingshijian %></td><td><%=faburen %></td><td><%=canjiaren %></td><td><%=beizhu %></td><td width='80' align='center'><%=issh%></td>
...
<p align="center">&nbsp;共<%=zgs%>条记录&nbsp;&nbsp;<%=page_record %>条/页　<a href="xianlujiatuan_list2.jsp?page=1">首页</a>　<a href="xianlujiatuan_list2.jsp?page=<%= curpage-1%>">上一页</a>　<A href="xianlujiatuan_list2.jsp?page=<%= curpage+1%>">下一页</A>　<a href="xianlujiatuan_list2.jsp?page=<%=zys %>">尾页</A>　当前第<FONT color=red><%=curpage %></FONT>页/共<FONT color=red><%=zys %></FONT>页</p>
  </body>
</html>

...
if(request.getParameter("nindexingming")=="" ||request.getParameter("nindexingming")==null ){}else{sql=sql+" and nindexingming like '%"+new String(request.getParameter("nindexingming").getBytes("8859_1"))+"%'";}
if(request.getParameter("nindedianhua")=="" ||request.getParameter("nindedianhua")==null ){}else{sql=sql+" and nindedianhua like '%"+new String(request.getParameter("nindedianhua").getBytes("8859_1"))+"%'";}
if(request.getParameter("nindeshenfenzheng")=="" ||request.getParameter("nindeshenfenzheng")==null ){}else{sql=sql+" and nindeshenfenzheng like '%"+new String(request.getParameter("nindeshenfenzheng").getBytes("8859_1"))+"%'";}
...
%>
  <tr>
    ...
    <td><%=jiudianmingcheng %></td><td><%=kefangbianhao %></td><td><%=jiage %></td><td><%=yudingshijian %></td><td><%=yudingtianshu %></td><td><%=nindexingming %></td><td><%=nindedianhua %></td><td><%=nindeshenfenzheng %></td><td><%=beizhu %></td><td><%=yonghuming %></td><td width='80' align='center'><a href="sh.jsp?id=<%=id%>&yuan=<%=issh%>&tablename=jiudianyuding" onClick="return confirm('您确定要执行此操作？')"><%=issh%></a></td>
...
<p align="center">&nbsp;共<%=zgs%>条记录&nbsp;&nbsp;<%=page_record %>条/页　<a href="jiudianyuding_list.jsp?page=1">首页</a>　<a href="jiudianyuding_list.jsp?page=<%= curpage-1%>">上一页</a>　<A href="jiudianyuding_list.jsp?page=<%= curpage+1%>">下一页</A>　<a href="jiudianyuding_list.jsp?page=<%=zys %>">尾页</A>　当前第<FONT color=red><%=curpage %></FONT>页/共<FONT color=red><%=zys %></FONT>页</p>
  </body>
</html>
